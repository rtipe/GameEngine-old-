...
namespace Uniti::Game {
    ...
    const ScenePath &SceneManager::getScenePath(const std::string &name) const {
        auto it = std::find_if(this->getAllScenes().begin(), this->getAllScenes().end(), [&](const ScenePath &path) {
    ...
    bool SceneManager::changeScene(const std::string &name) {
        auto it = std::find_if(this->getAllScenes().begin(), this->getAllScenes().end(),
    ...
    void SceneManager::update() {
        if (this->_nextScene != std::nullopt) {
            this->_currentScene = std::make_unique<Scene>(this->getSceneValue(this->_nextScene.value()), this->_nextScene->name);
    ...
    void SceneManager::init() {
        auto globalScenePath = this->getScenePath(Uniti::Game::Core::getProjectInfo().globalScene);
        auto startScenePath = this->getScenePath(Uniti::Game::Core::getProjectInfo().startScene);
        this->_globalScene = std::make_unique<Scene>(this->getSceneValue(globalScenePath), globalScenePath.name);
        this->_currentScene = std::make_unique<Scene>(this->getSceneValue(startScenePath), startScenePath.name);
    }
