...
namespace Uniti::Game {
    ...
    void Movement::update()
    {
        if (this->_isTo) {
            Render::Vector3f position = this->_object.getTransform().getPosition();
            Render::Vector2f normal =
                Render::Utils::Vector::getNormal({position.getX(), position.getY()}, this->_to);
    ...
    const Render::Clock &Movement::getClock() const
    {
        return this->_clock;
    }

    Render::Clock &Movement::getClock()
    {
        return this->_clock;
    }
