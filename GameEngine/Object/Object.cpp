          ...
          _scriptManager(object.getScriptManager().getData(), *this)
    {
        if (object.hasPrintable()) {
            if (object.getPrintable().getTypeName() == "sprite") {
                this->_printable =
                    std::make_unique<Render::Sprite>(dynamic_cast<Render::Sprite &>(object.getPrintable()));
            }
            if (object.getPrintable().getTypeName() == "text") {
                this->_printable =
                    std::make_unique<Render::Text>(dynamic_cast<Render::Text &>(object.getPrintable()));
            }
        }
    }
    ...
    void Object::setPrintable(const Json::Value &value)
    {
        ...
        if (type == "sprite") {
            ...
            std::unique_ptr<Render::Sprite> sprite =
                std::make_unique<Render::Sprite>(this->_scene.getAssetManager().getTexture(textureName));
        ...
        if (type == "text") {
            ...
            std::unique_ptr<Render::Text> text =
                std::make_unique<Render::Text>(this->_scene.getAssetManager().getFont(fontName));
