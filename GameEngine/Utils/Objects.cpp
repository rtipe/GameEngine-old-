...
Objects::find(const std::string &name, bool recursive) {
  auto global = find(
...
std::optional<std::reference_wrapper<Object>>
Objects::find(const std::string &name, const ObjectManager &objectManager,
              bool recursive) {
...
std::optional<std::reference_wrapper<Object>>
Objects::find_if(std::function<bool(const Object &object)> function,
                 bool recursive) {
...
std::optional<std::reference_wrapper<Object>>
Objects::find_if(const Scene &scene,
...
std::optional<std::reference_wrapper<Object>>
Objects::find_if(const ObjectManager &objectManager,
                 std::function<bool(const Object &)> function, bool recursive) {
  for (const auto &object : objectManager.getObjects()) {
    if (function(*object))
      return *object;
...
std::optional<std::reference_wrapper<Object>>
Objects::find_if(const Object &object,
                 std::function<bool(const Object &)> function, bool recursive) {
  auto child = find_if(object.getChildren(), function, recursive);
  if (child)
    return child;
