...
void MissileHandler::start() {
  this->getEvent().addEvent("createMissile", [&](const Json::Value &value) {
    for (const auto &data : value["data"]) {
      ...
      newMissile->getTransform().getPosition().setX(
          data["position"]["x"].asFloat());
      newMissile->getTransform().getPosition().setY(
          data["position"]["y"].asFloat());
      newMissile->setName(data["name"].asString());
      this->getEvent().addEvent(
          data["name"].asString(), [&](const Json::Value &value) {
            auto data = value["data"];
            auto exist = Uniti::Game::Utils::Objects::find(
                data["name"].asString(), this->getGameObject().getScene());
            if (exist) {
              exist.value().get().getTransform().getPosition().setX(
                  data["position"]["x"].asFloat());
              exist.value().get().getTransform().getPosition().setY(
                  data["position"]["y"].asFloat());
            }
          });
  ...
  this->getEvent().addEvent("destroyMissile", [&](const Json::Value &value) {
    for (const auto &name : value["data"]) {
      auto exist = Uniti::Game::Utils::Objects::find(
          name.asString(), this->getGameObject().getScene());
      if (exist)
