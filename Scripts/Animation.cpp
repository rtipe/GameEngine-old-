...
void Animation::update() {
    ...
    if (static_cast<float>(this->_clock.getMilliSeconds()) / 1000 < this->_animations[this->_actualAnimation].secondRemaining) return;

    Uniti::Render::Sprite &sprite = dynamic_cast<Uniti::Render::Sprite &>(this->getGameObject().getPrintable());
    this->_iteration++;
    if (this->_iteration >= this->_animations[this->_actualAnimation].frames.size()) {
    ...
    sprite.setTextureRect(this->_animations[this->_actualAnimation].frames[this->_iteration]);
    this->_clock.restart();
}
