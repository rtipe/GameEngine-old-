...
void Parallax::update() {
  for (auto &object : this->getGameObject().getChildren().getObjects()) {
    ...
    if (object->getTransform().getPosition().getX() <
        this->_end - this->_width) {
      float exceeded = (this->_end - this->_width) -
                       object->getTransform().getPosition().getX();
      object->getTransform().getPosition().setX(this->_width +
                                                (this->_end - exceeded - 2));
    }
  }
}
