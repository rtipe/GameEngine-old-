...
void MouseMovement::start() {
  this->getEvent().addEvent("Vessel", [&](const Json::Value &value) {
    for (const auto &nameVessel : value["data"].getMemberNames()) {
      ...
      std::vector<std::string> vesselType = {
          "BasicEnemy", "Kamikaze",   "Sniper",      "Tank",
          "Boss",       "VesselHeal", "VesselWeapon"};
      ...
      if (vessel == std::nullopt) {
        ...
        if (position.getX() !=
          ...
          vessel.value().get().getTransform().getPosition().setX(
              position.getX());
          vessel.value().get().getTransform().getPosition().setY(
              position.getY());
        }
      }
    }
  ...
  this->getEvent().addEvent("destroyEntity", [&](const Json::Value &value) {
    for (const auto &name : value["data"]) {
      ...
      auto it = std::find_if(noExplosion.begin(), noExplosion.end(),
...
void MouseMovement::update() {
  ...
  Uniti::Game::Core::getNetwork().getServer("game").sendEvent("vesselMovement",
                                                              position);
}
