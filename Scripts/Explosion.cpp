...
void Explosion::createExplosion(const Uniti::Render::Vector2f &position)
{
    ...
    newExplosion->getScriptManager().addScript(std::make_unique<Explosion>(*newExplosion), "Explosion");
    Uniti::Game::Core::getSceneManager().getCurrentScene().getObjects().add(std::move(newExplosion));
}
