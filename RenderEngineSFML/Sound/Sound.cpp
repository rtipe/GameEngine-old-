...
bool Sound::playSound(const std::string &name, float volume) {
  ...
  if (!this->_sounds.contains(name)) return false;
...
bool Sound::playMusic(const std::string &name, float volume, bool loop) {
  const std::lock_guard<std::mutex> lock(this->_mutex);
  if (!this->_musics.contains(name)) return false;
...
bool Sound::addSound(const Json::Value &value) {
  const std::lock_guard<std::mutex> lock(this->_mutex);
  if (!value.isMember("path") || !value.isMember("name")) return false;
  if (!value["path"].isString() || !value["name"].isString()) return false;
...
bool Sound::addSound(const std::string &name, const std::string &path) {
  ...
  if (!this->_soundsBuffer[name].loadFromFile(path)) return false;
...
bool Sound::addMusic(const std::string &name, const std::string &path) {
  ...
  if (!this->_musics[name]->openFromFile(path)) return false;
  return true;
}
}  // namespace Uniti::Render