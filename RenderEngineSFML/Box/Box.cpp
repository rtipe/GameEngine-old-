...
Box::Box(const Json::Value &value) {
  ...
  this->_dimension.set(value.get("width", 0).asFloat(),
                       value.get("height", 0).asFloat());
}

Box::Box(float x, float y, float left, float top)
    : _position(x, y), _dimension(left, top) {}

const Vector2f &Box::getPosition() const { return this->_position; }

const Vector2f &Box::getDimension() const { return this->_dimension; }

void Box::setPosition(const Vector2f &position) { this->_position = position; }

void Box::setDimension(const Vector2f &dimension) {
  this->_dimension = dimension;
}

void Box::setX(float x) { this->_position.setX(x); }

void Box::setY(float y) { this->_position.setY(y); }

void Box::setWidth(float width) { this->_dimension.setX(width); }

void Box::setHeight(float height) { this->_dimension.setY(height); }
...
bool Box::isInside(const Vector2f &point, const Vector3f &parent) const {
  Vector2f newPosition =
      Vector2f(parent.getX(), parent.getY()) + this->_position;
...
bool Box::isInside(const Box &box, const Vector3f &parentA,
                   const Vector3f &parentB) const {
  Vector2f newPositionA =
      Vector2f(parentA.getX(), parentA.getY()) + this->_position;
  ...
  Vector2f newPositionB =
      Vector2f(parentB.getX(), parentB.getY()) + box.getPosition();
