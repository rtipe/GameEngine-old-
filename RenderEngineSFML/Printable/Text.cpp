...
void Text::setColor(const Color &color) {
  const std::lock_guard<std::mutex> lock(this->_mutex);
  this->_text.setFillColor({static_cast<sf::Uint8>(color.getR()),
...
const std::string Text::getString() const { return this->_text.getString(); }

const std::string &Text::getTypeName() const { return this->_type; }

const Vector3f &Text::getPosition() const { return this->_position; }

const Vector2f &Text::getScale() const { return this->_scale; }

const float Text::getRotation() const { return this->_rotation; }
