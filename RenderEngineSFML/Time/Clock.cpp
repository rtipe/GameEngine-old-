...
long long Clock::getSeconds() const {
  auto current_time =
      (!_paused) ? std::chrono::high_resolution_clock::now() : _pause;
  auto duration =
      std::chrono::duration_cast<std::chrono::seconds>(current_time - _start);
...
void Clock::start() {
  ...
  if (_paused) {
    ...
    auto pause_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
        current_time - _pause);
