...
Uniti::Render::Vector2f Vector::getNormal(const Vector2i &base,
                                          const Vector2i &direction) {
  Vector2i normalVector = direction - base;
  Vector2f result(static_cast<float>(normalVector.getX()),
                  static_cast<float>(normalVector.getY()));
  auto length =
      std::sqrt(result.getX() * result.getX() + result.getY() * result.getY());
  if (length != 0) {
...
Uniti::Render::Vector2f Vector::getNormal(const Vector2f &base,
                                          const Vector2f &direction) {
  Vector2f normalVector = direction - base;
  auto length = std::sqrt(normalVector.getX() * normalVector.getX() +
...
Uniti::Render::Vector3f Vector::getNormal(const Vector3f &base,
